...
% Title
\def\thetitle{Multimedia Datenbanken --- Projekt 1}
...
\title{\textbf{\huge{\thetitle}}\\\Large\textsc{Team Amazonen}\\\large\textsc{Gruppe 2}}
\date{\today}
...
\begin{enumerate}
	\item \textit{Wie beeinflussen die Parameter $\alpha$ und $\beta$ das Ergebnis dieser Operation?}
		...
		Der Parameter $\alpha$ hat größere Auswirkungen auf Farben mit "großem" Pixelwert im Gegensatz zu Farbmischungen mit kleinem Pixelwert. Hellere Farben/Graustufen
		erfahren also eine größere Veränderung als dunkle.
		
		Der Parameter $\beta$ führt zu einer "Verschiebung" des Farb/Graustufenspektrums.
	\item \textit{Welche Probleme können auftreten, wenn $HK$ mit beliebigen Parametern durchgeführt wird? Schlagen Sie eine Methode vor, um damit umzugehen.}
	
	Bei beliebigen Parametern $\alpha$ und $\beta$ kann der errechnete Wert $P_{output}$ den Wertebereich für Pixel verlassen. Nachfolgend eine mögliche Normierung für
	8Bit-Pixelwerte
...
HK-Operation kann der Kontrast von G maximiert werden?}
	...
	Indem der maximale Farbraum ausgenutzt wird. Man führt eine sogenannte Grauwertstreckung durch. Geht man davon aus dass man einen Farbraum mit 255 Grauwerten hat, muss man die HK Transformation mit folgenden Werten durchführen:
	$$ \alpha = \frac{255}{b-a} $$
	$$ \beta = \frac{-255 \cdot a}{b-a} $$
	\item \textit{Implementieren Sie die HK-Operation, indem Sie die Methode hkImage der Klasse Model
vervollständigen, und die Bildumkehrfunktion (Button "Negativ").}

	\lstinputlisting[language=Java]{code/code.java}
\end{enumerate}
\end{document}
